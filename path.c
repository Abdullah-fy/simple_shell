...
/**
 * full_path -  checks a command if it is in a full path format or not
 *
 * @cmd: command path
 *
...
/**
 * file_exist - check a file if exists in path directories
 *
...
char *file_exist(char *path, char **env)
{
	...
	*dup = NULL, *pvalue = NULL;
	...
	else
	{
		...
		strtok(dup, "=");
		pvalue = strtok(NULL, " ");
		head = arg_list(pvalue, ":");
		temp = head;
		while (temp->next)
		{
			concat = str_concat(temp->str, path);
